...
static int exit_confirmed = 0; 
static int global_polling_enabled = 1;

static SceCtrlData global_controller;
...
void pmp_pause()
	...
	while (1)
		{
		SceCtrlData controller = global_controller;
...
char* pmp_read_vid_header( struct pmp_lib_vid_struct *pmp_vid_header, char* s )
{
	...
	proto  = net_io_isnet(s);
	if (FILE_IO == proto){
	    FILE* f = fopen(s, "rb");
	    if (f == 0){
		return("pmp_read_vid_header: can't open file");
	    }
	    if (fread(pmp_vid_header, 1, sizeof(struct pmp_lib_vid_struct), f) != sizeof(struct pmp_lib_vid_struct)){
		return("pmp_read_vid_header: can't read video header");
	    }
	    fclose(f);
	} else {
	    netfd *fd= net_io_open(s);
	    if (!fd)
	       return("pmp_read_vid_header: can't open file");
	    if  (net_io_read(fd,pmp_vid_header,sizeof(struct pmp_lib_vid_struct)) != sizeof(struct pmp_lib_vid_struct)){
		return("pmp_read_vid_header: can't read video header");
		net_io_close(fd);
	    }
	    net_io_close(fd);
...
int init(){
	...
	if (global_polling_enabled){
	    sceCtrlReadBufferPositive(&global_controller, 1);
	    if ( global_controller.Buttons & PSP_CTRL_HOME){
		exit_requested = 1;
		fprintf(stdout,"init: exit_requested = 1\n");
	    }
	}
	sceKernelDelayThread(10000);
    }
    fprintf(stdout,"init: delete thread\n");
...
void main_loop()
	...
	while (1)
		{
		exit_requested = 0;		
		fprintf(stdout,"enter mail loop exit_requested=%i\n",exit_requested);
	...
	result = opendir_open_net(&directory, net_directory);
		...
		struct pmp_lib_vid_struct *sorted_vid_struct=NULL;
		unsigned int total_size = 0;
		{
			...
			if (number_of_files)
			{
				...
				sorted_vid_struct =(struct pmp_lib_vid_struct*)malloc_64(sizeof(struct pmp_lib_vid_struct) * number_of_files);
				int	n = 0;
				...
				for (i = 0; i < directory.number_of_directory_entries; i++)
				{
						...
						(strcmp( name + len - 4, ".pmp") == 0))
					{
						...
						sorted_files[n] = directory.directory_entry[i].d_name;
					        pmp_read_vid_header(&sorted_vid_struct[n++], directory.directory_entry[i].d_name );
				...
				while (swap)
				{
					...
					for (i = 0; i < number_of_files - 1; i++)
					{
						if (strcmpupr( sorted_files[i] , sorted_files[i+1]) > 0)
						{
							...
							SceIoStat *tmp_stat = sorted_stats[i];
							sorted_stats[i] = sorted_stats[i+1];
							sorted_stats[i+1] = tmp_stat;
							
							char*	tmp_name = sorted_files[i];
							sorted_files[i] = sorted_files[i+1];
							sorted_files[i+1] = tmp_name;
							
							struct pmp_lib_vid_struct tmp_vid_struct = sorted_vid_struct[i];
							sorted_vid_struct[i] = sorted_vid_struct[i+1];
							sorted_vid_struct[i+1] = tmp_vid_struct;
		...
		while (1)
			...
			if (controller.Buttons & PSP_CTRL_CROSS || 
			    controller.Buttons & PSP_CTRL_TRIANGLE || 
			    controller.Buttons & PSP_CTRL_SQUARE  || 
			    controller.Buttons & PSP_CTRL_START || 
			    controller.Buttons & PSP_CTRL_CIRCLE || 
			    exit_requested )
				{
				break;
				}
			...
			vwidth = sorted_vid_struct[selected_entry].width;
			vheight= sorted_vid_struct[selected_entry].height;
			if (sorted_vid_struct[selected_entry].scale)
			    vrate = sorted_vid_struct[selected_entry].rate /  sorted_vid_struct[selected_entry].scale;
			else
			    vrate = sorted_vid_struct[selected_entry].rate;
			vframes =  sorted_vid_struct[selected_entry].number_of_frames;
		...
		if ( exit_requested )
			{
			exit_confirmed = 1;
			fprintf(stdout,"main_loop: exit_confirmed = 1\n");
			//-----------------
			free_64(sorted_files);
			free_64(play_list.play_list_entry);
			free_64(sorted_vid_struct);
			//-----------------
			opendir_close(&directory);
		...
		#endif
		global_polling_enabled = 0;
		result = pmp_play(sorted_files[play_entry], usePos);
		global_polling_enabled = 1;
		if (result != 0)
			{
			sceKernelDelayThread(1000000);
			print_error(result);
