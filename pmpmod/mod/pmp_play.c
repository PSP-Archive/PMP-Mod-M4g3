...
void blitSpeed ( int dx, int dy, int speed )
{
	...
	i2=(speed%100)/10;
	i3=speed%10;
	spriteBlit( dx, dy, FONT_START_X + FONT_STEP*i1, FONT_START_Y, FONT_WIDTH, FONT_HEIGHT );
	spriteBlit( dx + FONT_WIDTH, dy, FONT_START_X + FONT_STEP*i2, FONT_START_Y, FONT_WIDTH, FONT_HEIGHT );
	spriteBlit( dx + (FONT_WIDTH*2), dy, FONT_START_X + FONT_STEP*i3, FONT_START_Y, FONT_WIDTH, FONT_HEIGHT );
	...
	#define BLIT_SLICE	32
		//		advancedBlit(0,0,480,272,0,0,480,272,16);		// copy full buffer to full screen
				...
				if (p->interface_pos)
				{
					...
					blitSpeed( 480 - 48 , 272-16+y, net_io_read_get_speed());
...
char *pmp_play_open(struct pmp_play_struct *p, char *s, int usePos)
	...
	p->input_thread = sceKernelCreateThread("input", pmp_input_thread, 0x8, 0x10000, 0, 0);
	if (p->input_thread < 0)
		{
		pmp_play_close(p,0);
